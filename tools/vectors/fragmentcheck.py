...
class FragmentCheck(VulnerabilityVector):
        ...
        def analyze(self):
            ...
            list_Fragment = self.context.filteringEngine.filter_list_of_classes(list_Fragment)
            ...
            list_Fragment_vulnerability_NonMethod_classes = self.context.filteringEngine.filter_list_of_classes(
                list_Fragment_vulnerability_NonMethod_classes)
            list_Fragment_vulnerability_Method_OnlyReturnTrue_methods = self.context.filteringEngine.filter_list_of_methods(
                list_Fragment_vulnerability_Method_OnlyReturnTrue_methods)
            list_Fragment_vulnerability_Method_NoIfOrSwitch_methods = self.context.filteringEngine.filter_list_of_methods(
                list_Fragment_vulnerability_Method_NoIfOrSwitch_methods)
        ...
        (6)https://cureblog.de/2013/11/cve-2013-6271-remove-device-locks-from-android-phone/ """
                ...
                self.context.writer.startWriter("FRAGMENT_INJECTION", LEVEL_CRITICAL, "Fragment Vulnerability Checking", output_string, None,
                                   "BID 64208, CVE-2013-6271")

                if list_Fragment_vulnerability_NonMethod_classes:
                    if self.context.int_target_sdk >= 19:
                ...
                if list_Fragment:
                    self.context.writer.write("All of the potential vulnerable \"fragment\":")
                    for i in list_Fragment:
                        self.context.writer.write("    " + i)

            else:
                self.context.writer.startWriter("FRAGMENT_INJECTION", LEVEL_INFO, "Fragment Vulnerability Checking",
